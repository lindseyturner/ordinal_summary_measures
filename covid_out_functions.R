...
# there are separate functions for adjusted and unadjusted models 

# very similar to simulation functions, but output data in a list instead of a vector 
# new function for weighted risk difference 
# assumes a partial proportional odds model from the rmsb package
# assumes the treatment is coded as -0.5 for control and 0.5 for treatment 
# unadjusted model
# b is the model
# iprior is the value of iprior in the rmsb model
# dat is the data 
# trt is the name of the treatment variable in the data 
# outcome is the name of the outcome variable in the data
...
# calculate the credible interval for the unadjusted model, again very similar to simulation function
# assumes a partial proportional odds model from the rmsb package
# assumes the treatment is coded as -0.5 for control and 0.5 for treatment 
# NOTE: this does not work for iprior = 1
# b is the model
...
order_weighted_RD2_CI <- function(b, n_post_draws = 4000, dat, trt, outcome) { 
  ...
  denominator <- sum(p_y_ctl_est[j]) + sum(p_y_ctl_est[k])
  weighted_RD_B <- numerator/denominator
  weighted_RD_B95CI <- quantile(weighted_RD, probs = c(0.025, 0.975))
  weighted_RD_Bp <- 1 - sum(weighted_RD >= 0)/n_post_draws
  weighted_RD_Bsd <- sd(weighted_RD)
  ...
  denominator <- sum(p_y_ctl_est[j]) + sum(p_y_ctl_est[k])
  weighted_OR_B <- exp(numerator/denominator)
  weighted_OR_B95CI <- quantile(weighted_OR, probs = c(0.025, 0.975))
  weighted_OR_Bp <- 1 - sum(weighted_OR >= 1)/n_post_draws
  weighted_OR_Bsd <- sd(weighted_OR)
  
  weighted_log_OR_B <- numerator/denominator
  weighted_log_OR_B95CI <- quantile(weighted_log_OR, probs = c(0.025, 0.975))
  weighted_log_OR_Bp <- 1 - sum(weighted_log_OR >= 1)/n_post_draws
  weighted_log_OR_Bsd <- sd(weighted_log_OR)
  ...
  denominator <- sum(p_y_ctl_est[j]) + sum(p_y_ctl_est[k])
  weighted_log_RR_Bcontrol <- numerator/denominator
  weighted_log_RR_B95CI_control <- quantile(weighted_log_RR_control, probs = c(0.025, 0.975))
  weighted_log_RR_Bp_control <- 1 - sum(weighted_log_RR_control >= 0)/n_post_draws
  weighted_log_RR_Bsd_control <- sd(weighted_log_RR_control)
...
## Adjusted Partial Proportional Odds model 
# new function for weighted summary measures with an adjusted model 
# assumes an adjusted partial proportional odds model from rmsb
# assumes the treatment is coded as -0.5 for control and 0.5 for treatment 
# Uses Bayesian Bootstrap to get average 
# note in the model, the first covariate must be treatment arm and then the other covariates we are adjusting for
# b is the model
# iprior is the value of iprior in the rmsb model
# dat is the data 
# trt is the name of the treatment variable in the data 
...
# assumes Bayesian partial proportional odds model from rmsb package
# treatment should be first covariate in the model
# treatment should be coded as -0.5 for control and 0.5 for treatment
# this function does not work if iprior = 1
# b is the model
# n_post_draws is the number of posterior draws used for the model
# dat is the data 
# iprior is the value of iprior in the rmsb model
# outcome is the name of the outcome variable in the data
# trt is the name of the treatment variable in the data 
...
# new function for weighted risk difference 
# assumes a Bayesian unadjusted proportional odds model from rmsb package
# assumes the treatment is coded as -0.5 for control and 0.5 for treatment 
# b is the model
# dat is the data 
# trt is the name of the treatment variable in the data 
...
# assumes a Bayesian unadjusted partial proportional odds model from rmsb package
# assumes the treatment is coded as -0.5 for control and 0.5 for treatment 
# b is the model
...
# Bayesian proportional odds model (from rmsb package) adjusting for other covariates
# new function for weighted risk difference with an adjusted model 
# assumes a proportional odds model 
# assumes the treatment is coded as -0.5 for control and 0.5 for treatment 
# use bayesian bootstrap to combine over individuals
# b is the model
# c is the number of levels in the outcome variable
# dat is the data 
# trt_variable is the name of the treatment variable in the data 
# outcome is the name of the outcome variable in the data
...
# new function for adjusted CI for a Bayesian proportional odds model from rmsb package
# using Bayesian bootstrap to combine over individuals
# assumes treatment is coded as -0.5 for control and 0.5 for treatment
# b is the model
# c is the number of levels in the outcome variable
# n_post_draws is the number of posterior draws in the model
# dat is the data 
...
# write function to calculate order invariant weighted summary measures using polr 
# b is the model 
# c is the number of levels in the outcome variable
